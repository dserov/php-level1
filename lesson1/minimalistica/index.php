...
$title = "minimalistica";
$h1 = "Заголовок страницы 1-го уровня";
$currYear = date("Y");
...
function showMenu($topicName, $menu)
{
    if ($topicName) {
        echo '<li ' .
            (empty($menu['id']) ? '' : sprintf('id="%s" ', $menu['id'])) .
            (empty($menu['class']) ? '' : sprintf('class="%s_item" ', $menu['class'])) .
            (empty($menu['title']) ? '' : sprintf('title="%s" ', $menu['title'])) .
            '>';
        echo '<a ' .
            (empty($menu['href']) ? '' : sprintf('href="%s" ', $menu['href'])) .
            (empty($menu['class']) ? '' : sprintf('class="%s_item-link" ', $menu['class'])) .
            (empty($menu['title']) ? '' : sprintf('title="%s" ', $menu['title'])) .
            '>' . $topicName . '</a>';
    }
    if (isset($menu['items']) && is_array($menu['items'])) {
        echo PHP_EOL . '<ul ' .
            (empty($menu['id']) ? '' : sprintf('id="%s" ', $menu['id'])) .
            (empty($menu['class']) ? '' : sprintf('class="%s" ', $menu['class'])) .
            (empty($menu['title']) ? '' : sprintf('title="%s" ', $menu['title'])) .
            '>' . PHP_EOL;
